...
class CV:
    ...
    def __init__(self, **config):
        ...
        self.stage = "lateral" #we center laterally first, stage for centering onto bin 
        self.drop = False
    ...
    ########
    def search(self, frame, target, detections):
            self.wait = True
            #calls centering if phase is set, else we search
            lateral = 0 #default
            forward = 0 #default
    ...
    def run(self, frame, target, detections):
        ...
        for i in len(detections):
            if detections(i).label == "Fire" or detections(i).label == "fire":
                detections = []
                break

        if len(detections) == 0 and self.phase is None: #We are going to search
            lateral, forward = search(frame, target, detections)
        ...
        if len(detections) >= 1:
            self.phase = 'centering' #now we get to centering baby :)
            if len(detections) == 1:
        ...
        if self.state == "missed": 
            ...
            if self.missed_frames >= 100: #100 missed frames
                self.missed_frames = 0
                self.phase = None #go back to searching
        ...
        return motion_commands_and_state, frame #end of run()

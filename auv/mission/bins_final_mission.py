...
class BinsDropMission:
    cv_files = ["bins_final_cv"] # CV file to run
    ...
    def __init__(self, target=None, **config):
        ...
        self.cv_handler.set_target("bins_final_cv", target)
    ...
    def run(self):
        ...
        while not rospy.is_shutdown():
            ...
            lateral = self.data["bins_final_cv.py"].get("lateral", None)
            forward = self.data["bins_final_cv.py"].get("forward", None)
            yaw = self.data["bins_final_cv.py"].get("yaw", None)
            vertical = self.data["bins_final_cv.py"].get("vertical", None)
            drop = self.data["bins_final_cv.py"].get("drop", None)
            wait = self.data["bins_final_cv.py"].get("wait", None)
            ...
            if drop == True:
                ...
                self.robot_control.movement(forward=forward, lateral=lateral)
...
if __name__ == "__main__":
    ...
    rospy.init_node("bins_final_cv", anonymous=True)

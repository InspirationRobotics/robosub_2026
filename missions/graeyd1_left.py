...
with open("./missions/waypoints/33left.json", "r") as file:
    data = json.load(file)
...
rc.go_to_depth(.6)
rospy.loginfo("Robot armed and set to depth .6 m")
...
try:
   ...
   comms.rec_callback()
   navigate_to("G1")
   comms.send_repeated(dest_addr=ONYX_ADDR,message="GATE_START",count=5,delay=0.5)
   rospy.loginfo("Passed through gate")
   comms.send_repeated(dest_addr=ONYX_ADDR,message="GATE_FINISH",count=5,delay=0.5)
...
try:
    ...
    comms.send_repeated(dest_addr=ONYX_ADDR,message="SLALOM_START",count=5,delay=0.5)
    navigate_to("S1")
    navigate_to("S2")
    #navigate_to("S3")
    comms.send_repeated(dest_addr=ONYX_ADDR,message="SLALOM_FINISH",count=5,delay=0.5)
    #navigate_to("S4")
...
try:
    ...
    comms.send_repeated(dest_addr=ONYX_ADDR,message="RETURN_START",count=5,delay=0.5)
    navigate_to("R1")
    navigate_to("R2")
    navigate_to("R3")
    comms.send_repeated(dest_addr=ONYX_ADDR,message="RETURN_FINISH",count=5,delay=0.5)
